...
graph twoway line normalized_n_ocu trim , lwidth(thick)	/*
		...
		*/ 	legend(on cols(1) label(1 "Ocupações qualificadas e de liderança") size(Small) forcesize symysize(2pt) symxsize(2pt) ) 	/*
		*/ 	xlabel(#8, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_dirigentes", replace) 
		
graph use "$tmp_dir\_graph_dirigentes.gph"		
erase "$tmp_dir\_graph_dirigentes.gph"
graph export "$output_dir\_graph_dirigentes.png", replace	
...
graph use "$tmp_dir\_graph_dirigentes_desagregado.gph"		
erase "$tmp_dir\_graph_dirigentes_desagregado.gph"
graph export "$output_dir\_graph_dirigentes_desagregado.png", replace					

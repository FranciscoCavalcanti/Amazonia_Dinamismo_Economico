...
graph twoway line normalized_n_ocu trim , lwidth(thick)	/*
		...
		*/ 	legend(on cols(1) label(1 "Leadership and highly skilled occupations") size(Small) forcesize symysize(2pt) symxsize(2pt) ) 	/*
		*/ 	xlabel(#8, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_qualificadas_amz_legal_english", replace) 
		
graph use "$tmp_dir\_graph_qualificadas_amz_legal_english.gph"		
erase "$tmp_dir\_graph_qualificadas_amz_legal_english.gph"
graph export "$output_dir\_graph_qualificadas_amz_legal_english.png", replace	
...
graph twoway line normalized_n_ocu trim  if titulo == "Cientistas e engenheiros", lwidth(thick) || /*
		...
		*/ 	lwidth(thick) 	/*		
		*/	yscale( axis(1) range(90 170) lstyle(none) )	/* how y axis looks
		*/ 	legend(on cols(1) label(1 "Scientists and engineers") label(2 "Directors and managers") label(3 "Administrators and management specialists") size(Small) forcesize symysize(2pt) symxsize(2pt) ) 	/*
		*/ 	xlabel(#9, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_qualificadas_amz_legal_desagregado_english", replace) 
				
graph use "$tmp_dir\_graph_qualificadas_amz_legal_desagregado_english.gph"		
erase "$tmp_dir\_graph_qualificadas_amz_legal_desagregado_english.gph"
graph export "$output_dir\_graph_qualificadas_amz_legal_desagregado_english.png", replace					

...
gen titulo = "Total de ocupações relativas à serviços"
tempfile base_total
...
#delim ;    
esttab matrix(A, fmt("%16,0fc" "%16,1fc" "%16,1fc" "%16,0fc" "%16,0fc" "%16,1fc" "%16,1fc")) using "$output_dir\tableservicosamzlegaledul.tex", 
	replace 
    prehead(
		"\begin{table}[H]"
		"\centering"
		"\label{tableservicosamzlegaledul}"
		"\scalebox{0.56}{"
	...
	coeflabels(   /* run the follwing code:  label list nova_agregacao */
          ...
          30 "Vendedores"
          31 "Total de ocupações relativas à serviços"	  
		  )
    ;
#delim cr
...
* call data 
use "$input_dir\_amz_manaus_numero_ocupados_por_setor.dta", clear
gen id = cod_setor
sort Ano Trimestre 
...
collapse (mean) massa_salarial, by (cod_setor Ano)

drop if cod_setor=="."

sort cod_setor Ano

by cod_setor, sort: gen tx_crescimento = (massa_salarial[_n]/massa_salarial[_n-1])*100 -100
...
* local notes
local ttitle "Taxas de crescimento de massa de rendimentos por setores econômicos entre 2012 e 2019"
local tnotes "Fonte: com base nos dados da PNAD Contínua, IBGE"

#delim ;    
esttab matrix(A, fmt(%16,2fc)) using "$output_dir\amzmanausrkngtxmassaporsetor.tex", 
    replace 
    collabels("Tx. Cresc. (\%)")
    prehead(
        "\begin{table}[H]"

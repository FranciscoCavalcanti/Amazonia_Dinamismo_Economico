...
* call data 
use "$input_dir\_amz_manaus_numero_ocupados_por_ocupacao.dta", clear
gen id = cod_ocupacao
sort Ano Trimestre 
...
collapse (mean) renda_media, by (cod_ocupacao Ano)
...
by cod_ocupacao, sort: gen tx_crescimento = (renda_media[_n]/renda_media[_n-1])*100 -100
...
* local notes
local ttitle "Taxas de crescimento de rendimentos médio por tipo de ocupação entre 2012 e 2019"
local tnotes "Fonte: com base nos dados da PNAD Contínua, IBGE"

#delim ;    
esttab matrix(A, fmt(%16,2fc)) using "$output_dir\amzmanausrkngtxrendaporocupacao.tex", 
	replace 

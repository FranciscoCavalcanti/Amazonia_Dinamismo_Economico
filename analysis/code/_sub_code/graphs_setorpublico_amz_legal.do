...
keep if titulo == "Profissionais da saúde" /* Profissionais da saúde
	...
	*/ 	| titulo == "Profissionais do ensino"  	/* Profissionais do ensino
	*/ 
...
graph twoway line normalized_n_ocu trim , lwidth(thick)	/*
		...
		*/ 	xlabel(#8, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_setorpublico_amz_legal", replace) 
		
graph use "$tmp_dir\_graph_setorpublico_amz_legal.gph"		
erase "$tmp_dir\_graph_setorpublico_amz_legal.gph"
graph export "$output_dir\_graph_setorpublico_amz_legal.png", replace	
...
graph twoway line normalized_n_ocu trim  if titulo == "Profissionais da saúde", lwidth(thick) || /*
		*/ 	line normalized_n_ocu trim  if titulo == "Policiais, bombeiros e forças armadas", lwidth(thick) || /*
		*/ 	line normalized_n_ocu trim  if titulo == "Profissionais do ensino", lwidth(thick)	/*
		*/ 	title("", size(Medium)) 	/*
		*/	graphregion(fcolor(white)) 	/*
		*/ 	ytitle("") 	/*
		*/ 	xtitle("")	/*	
		...
		*/ 	xlabel(#9, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_setorpublico_amz_legal_desagregado", replace) 
				
graph use "$tmp_dir\_graph_setorpublico_amz_legal_desagregado.gph"		
erase "$tmp_dir\_graph_setorpublico_amz_legal_desagregado.gph"
graph export "$output_dir\_graph_setorpublico_amz_legal_desagregado.png", replace					

...
* call data 
use "$input_dir\_amz_metropolitana_numero_ocupados_por_atividade_2digitos.dta", clear

gen id = cod_cnae2dig
sort Ano Trimestre cod_cnae2dig id
...
collapse (mean) n_ocu_cnae, by (cod_cnae2dig Ano)
...
by cod_cnae2dig, sort: gen delta_n = (n_ocu_cnae[_n] - n_ocu_cnae[_n-1])

collapse (mean) delta_n, by (cod_cnae2dig)
sort delta_n
...
mkmat delta_n, matrix(A) rownames(cod_cnae2dig)

* local notes
local ttitle "Variação absoluta do número de ocupações por grupamentos de atividade entre 2012 e 2019"
local tnotes "Fonte: com base nos dados da PNAD Contínua, IBGE"

#delim ;    
esttab matrix(A, fmt(%16,0fc)) using "$output_dir\amzmetropolitanarkngnocuporcnae2dig.tex", 
	replace 
	collabels("Variação")
    prehead(
        "\begin{table}[H]"
        "\centering"
		"\label{amzmetropolitanarkngnocuporcnae2dig}"
		"\scalebox{0.70}{"

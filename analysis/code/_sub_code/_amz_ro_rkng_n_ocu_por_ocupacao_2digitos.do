...
* call data 
use "$input_dir\_amz_ro_numero_ocupados_por_ocupacao_2digitos.dta", clear
...
#delim ;    
esttab matrix(A, fmt(%16,0fc)) using "$output_dir\amzrorkngnocuporcod2dig.tex", 
	replace 
	collabels("Variação")
    prehead(
        "\begin{table}[H]"
        "\centering"
		"\label{amzrorkngnocuporcod2dig}"
		"\scalebox{0.70}{"

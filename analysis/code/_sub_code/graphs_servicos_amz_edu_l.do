...
* call data 
use "$input_dir\_amz_edu_l_numero_ocupados_por_ocupacao_2digitos.dta", clear
...
graph twoway line normalized_n_ocu trim , lwidth(thick)	/*
		...
		*/ 	lwidth(thick) 	/*		
		*/	yscale(axis(1) range(20 180) lstyle(none) )	/* how y axis looks
		*/ 	legend(on cols(2) label(1 "Ocupações relativas à serviços") size(Small) forcesize symysize(2pt) symxsize(2pt) ) 	/*
		*/ 	xlabel(#8, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_servicos_amz_edu_l", replace) 
		
graph use "$tmp_dir\_graph_servicos_amz_edu_l.gph"		
erase "$tmp_dir\_graph_servicos_amz_edu_l.gph"
graph export "$output_dir\_graph_servicos_amz_edu_l.png", replace	
...
graph twoway line normalized_n_ocu trim  if titulo == "Serviços e cuidados pessoais", lwidth(thick) || /*
		...
		*/ 	xlabel(#9, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_servicos_amz_edu_l_desagregado", replace) 
				
graph use "$tmp_dir\_graph_servicos_amz_edu_l_desagregado.gph"		
erase "$tmp_dir\_graph_servicos_amz_edu_l_desagregado.gph"
graph export "$output_dir\_graph_servicos_amz_edu_l_desagregado.png", replace					

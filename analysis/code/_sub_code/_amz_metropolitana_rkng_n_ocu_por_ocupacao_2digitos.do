...
* call data 
use "$input_dir\_amz_metropolitana_numero_ocupados_por_ocupacao_2digitos.dta", clear

gen id = cod_cod2dig
sort Ano Trimestre cod_cod2dig id
...
collapse (mean) n_ocu_cod, by (cod_cod2dig Ano)
...
by cod_cod2dig, sort: gen delta_n = (n_ocu_cod[_n] - n_ocu_cod[_n-1])

collapse (mean) delta_n, by (cod_cod2dig)
...
tostring cod_cod2dig, replace
destring delta_n, replace

// transforma data em matrix
mkmat delta_n, matrix(A) rownames(cod_cod2dig)
...
#delim ;    
esttab matrix(A, fmt(%16,0fc)) using "$output_dir\amzmetropolitanarkngnocuporcod2dig.tex", 
	replace 
	collabels("Variação")
    prehead(
        "\begin{table}[H]"
        "\centering"
		"\label{amzmetropolitanarkngnocuporcod2dig}"
		"\scalebox{0.70}{"

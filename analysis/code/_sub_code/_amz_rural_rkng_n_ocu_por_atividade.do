...
sort Ano Trimestre cod_atividade id
...
by cod_atividade, sort: gen delta_n = (n_ocu_cnae[_n] - n_ocu_cnae[_n-1])

collapse (mean) delta_n, by (cod_atividade)
...
* cap drop cod_atividade
tostring cod_atividade, replace
...
#delim ;    
esttab matrix(A, fmt(%16,0fc)) using "$output_dir\amzruralrkngnocuporatividade.tex", 
	replace 
	collabels("Variação")
    prehead(
        "\begin{table}[H]"

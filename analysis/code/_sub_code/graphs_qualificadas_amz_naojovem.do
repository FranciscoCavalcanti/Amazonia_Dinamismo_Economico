...
graph twoway line normalized_n_ocu trim , lwidth(thick)	/*
		...
		*/ 	xlabel(#8, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_qualificadas_amz_naojovem", replace) 
		
graph use "$tmp_dir\_graph_qualificadas_amz_naojovem.gph"		
erase "$tmp_dir\_graph_qualificadas_amz_naojovem.gph"
graph export "$output_dir\_graph_qualificadas_amz_naojovem.png", replace	
...
graph twoway line normalized_n_ocu trim  if titulo == "Cientistas e engenheiros", lwidth(thick) || /*
		...
		*/ 	lwidth(thick) 	/*		
		*/	yscale( axis(1) range(90 170) lstyle(none) )	/* how y axis looks
		*/ 	legend(on cols(1) label(1 "Cientistas e engenheiros") label(2 "Dirigentes e gerentes") label(3 "Administradores e especialistas em gestão") size(Small) forcesize symysize(2pt) symxsize(2pt) ) 	/*
		*/ 	xlabel(#9, angle(45)) 	/*
		*/  saving("$tmp_dir\_graph_qualificadas_amz_naojovem_desagregado", replace) 
				
graph use "$tmp_dir\_graph_qualificadas_amz_naojovem_desagregado.gph"		
erase "$tmp_dir\_graph_qualificadas_amz_naojovem_desagregado.gph"
graph export "$output_dir\_graph_qualificadas_amz_naojovem_desagregado.png", replace					
